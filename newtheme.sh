...
echo "${boldwhite}Please note: This script is designed to work with dudestack and miljon. See https://github.com/digitoimistodude${txtreset} "
...
for i in `grep -rl miljon * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;miljon;${THEMENAME};" $i $i; done
for i in `grep -rl miljon * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;miljon;${THEMENAME};" $i $i; done
for i in `grep -rl air * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;miljon;${THEMENAME};" $i $i; done
...
On top of features included in [digitoimistodude/miljon](https://github.com/digitoimistodude/miljon), this project contains:
